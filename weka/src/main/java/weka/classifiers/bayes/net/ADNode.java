...
public class ADNode implements Serializable, TechnicalInformationHandler,
	...
	public static VaryNode makeVaryNode(int iNode, ArrayList<Integer> nRecords,
			Instances instances) {
		VaryNode varyNode = new VaryNode(iNode);
		...
		varyNode.m_nMCV = nMCV;

		// determine child nodes
		varyNode.m_ADNodes = new ADNode[nValues];
		for (int iChild = 0; iChild < nValues; iChild++) {
			if (iChild == nMCV || nChildRecords[iChild].size() == 0) {
				varyNode.m_ADNodes[iChild] = null;
			} else {
				varyNode.m_ADNodes[iChild] = makeADTree(iNode + 1,
	...
	public static ADNode makeADTree(int iNode, ArrayList<Integer> nRecords,
		...
		adNode.m_nCount = nRecords.size();
		adNode.m_nStartNode = iNode;
		if (nRecords.size() < MIN_RECORD_SIZE) {
			adNode.m_Instances = new Instance[nRecords.size()];
			for (int iInstance = 0; iInstance < nRecords.size(); iInstance++) {
				adNode.m_Instances[iInstance] = instances.instance(nRecords.get(
						iInstance).intValue());
			}
		} else {
			adNode.m_VaryNodes = new VaryNode[instances.numAttributes() - iNode];
			for (int iNode2 = iNode; iNode2 < instances.numAttributes(); iNode2++) {
				adNode.m_VaryNodes[iNode2 - iNode] = makeVaryNode(iNode2, nRecords,
						instances);
			}
		}
		return adNode;
