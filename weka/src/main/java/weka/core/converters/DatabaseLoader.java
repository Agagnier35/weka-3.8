...
public class DatabaseLoader extends AbstractLoader implements BatchConverter,
	...
	public String globalInfo() {
		return "Reads Instances from a Database. "
				...
				+ "If this extension is impossible instances will be loaded pseudoincrementally. To ensure that every row is fetched exaclty once, they have "
				+ "to ordered.\n"
				+ "Therefore a (primary) key is necessary.This approach is chosen, instead of using JDBC driver facilities, because the latter one differ "
				+ "betweeen different drivers.\n"
				+ "If you use the DatabaseSaver and save instances by generating automatically a primary key (its name is defined in DtabaseUtils), this "
				+ "primary key will "
				+ "be used for ordering but will not be part of the output. The user defined SQL query to extract the instances should not contain LIMIT and "
				+ "ORDER BY clauses (see -Q option).\n"
				+ "In addition, for incremental loading,  you can define in the DatabaseUtils file how many distinct values a nominal attribute is allowed to "
							...
							System.out
									.println(
											"Incremental loading not supported for that DBMS. Pseudoincremental mode is used if you use incremental loading"
													+ ".\nAll rows are loaded into memory once and retrieved incrementally from memory instead of from the "
													+ "database.");
				...
				if (!checkForKey()) {
					throw new Exception(
							"A unique order cannot be detected automatically.\nYou have to use SELECT * in your query to enable this feature.\nMaybe JDBC "
									+ "driver is not able to detect key.\nDefine primary key in your database or use -P option (command line) or enter key "
									+ "columns in the GUI.");
				}
			}
			if (m_firstTime) {
