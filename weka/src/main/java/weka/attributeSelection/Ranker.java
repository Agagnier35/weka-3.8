...
public class Ranker extends ASSearch implements RankedOutputSearch,
	...
	public int[] search(ASEvaluation asEvaluator, Instances data) throws Exception {
		...
		if (asEvaluator instanceof UnsupervisedAttributeEvaluator) {
			m_hasClass = false;
		} else {
		...
		if (asEvaluator instanceof AttributeTransformer) {
			data = ((AttributeTransformer) asEvaluator).transformedHeader();
		...
		AttributeEvaluator ASEvaluator = (AttributeEvaluator) asEvaluator;
